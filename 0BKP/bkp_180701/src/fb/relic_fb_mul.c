...
void fb_mul_lodah(fb_t c, const fb_t a, const fb_t b) {
	...
	TRY {
		...
		volatile int* a_copy = (int*) ADDR_A;
		volatile int* b_copy = (int*) ADDR_B;
		volatile int* c_copy = (int*) ADDR_C;

 		int* addr_t    = (int*) ADDR_T;
 		int* addr_ir   = (int*) ADDR_IR;
 		int* addr_ir_t = (int*) ADDR_IR_T;
/*
...
*/
		...
		uint8_t Idrb, Idrt, Idrir, Idrc, Idrirt;
 		...
 		Idrc =   (((unsigned int)ADDR_C)    >> 8 ) & 0x7F;
 		Idrt =   (((unsigned int)addr_t)    >> 8 ) & 0x7F;
